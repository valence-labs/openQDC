...
def read_mol(mol_h5, mol_name, energy_target_names, force_target_names):
    ...
    forces = np.concatenate([np.stack([m[c][f_tag] 
                                        for f_tag in force_target_names], axis=-1)
                                        for c in cids], axis=0)
    energies = np.stack([np.array([m[c][e_tag][0] 
                                        for e_tag in energy_target_names]) 
                                        for c in cids], axis=0)
...
class QM7X(BaseDataset):
    ...
    def read_raw_entries(self):
        ...
        for i in range(1, 9):
            ...
            samples += [read_mol(data[k], k, self.energy_target_names, self.force_target_names) 
